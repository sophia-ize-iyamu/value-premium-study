...
1963 to present. It asks whether the value premium is still positive and significant,
whether it's stable or regime-dependent, and what a purely backward-looking estimate
honestly implies about forward expectations.

The point is as much about method as about value: treating an in-sample factor mean as
a forward forecast is a classic error, and this note is built to show the gap.

The notebook is committed with its outputs, so it renders in full (charts, tables, and
the printed results) right here on GitHub. No setup is needed just to read it.
